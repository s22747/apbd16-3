...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = emps.Where(e => e.DeptNo.Equals(30)).ToList();
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(e => e.DeptNo.Equals(depts.Where(e => e.Loc.Equals("CHICAGO")))).ToList();
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = emps.Join(depts, emp => emp.DeptNo, dept => dept.DeptNo, (emp, dept) => new {EName = emp.EName, DName = dept.DName}).ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps
        .SelectMany(e => grades, (e, s) => new { Emp = e, Grade = s })
        .Where(x => x.Emp.Sal >= x.Grade.Losal && x.Emp.Sal <= x.Grade.Hisal)
        .Select(x => new { x.Emp.EName, x.Grade.Grade });
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        .GroupBy(e => e.DeptNo)
        .SelectMany(g =>
            g.Where(e => e.Sal > g.Average(x => x.Sal))
        )
        .Select(e => e.EName);
